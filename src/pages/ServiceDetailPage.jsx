...
import { ArrowLeft, Play, MapPin, Clock, Users, Video, Camera } from 'lucide-react';
            ...
            <div className="bg-gray-900 rounded-lg p-8 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#FF9500] font-bold text-lg">{new Intl.NumberFormat('vi-VN').format(service.amount)} VND</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#FF9500] font-semibold">{service.areas.map(area => getLabel(area, SERVICE_AREAS)).join(', ')}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#FF9500] font-semibold">{service.categories.map(cat => getLabel(cat, SERVICE_CATEGORIES)).join(' / ')}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-6 mb-8">
                <div className="flex items-center space-x-2 text-gray-400 text-sm">
                  ...
                  <span>Thời gian nộp hồ sơ: {new Date(service.date_get_job).toLocaleDateString('vi-VN')}</span>
                </div>
                <div className="flex items-center space-x-2 text-gray-400 text-sm">
              ...
              <div className="mb-8">
                <h3 className="text-white text-xl font-bold mb-4 border-l-4 border-[#FF9500] pl-3">Chi tiết gói dịch vụ</h3>
          ...
          <div className="w-full lg:w-96">
            <div className="bg-gray-900 rounded-lg p-6 mb-6">
              <div className="text-center mb-6">
                ...
                <h3 className="text-white text-xl font-bold mb-1">{cameraman.full_name}</h3>
              </div>

              <button className="w-full bg-[#FF9500] hover:bg-[#FF9500] text-black font-bold py-3 rounded-lg mb-6">
                Đã xác minh
              </button>

              <div className="space-y-4 text-sm text-gray-300 mb-6">
                <div>
                  <strong className="text-white">Cinematic Videographer</strong><br />
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {service.styles.map((style, index) => (
                    <span key={index} className={`px-3 py-1.5 text-xs font-semibold rounded-full ${index === 0 ? 'bg-[#FF9500] text-white' : 'bg-gray-700 text-gray-300'}`}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {service.categories.map((category, index) => (
                    <span key={index} className="px-3 py-1.5 bg-gray-700 text-gray-300 text-xs font-medium rounded-full uppercase">
            ...
            <div className="bg-gray-900 rounded-lg p-6">
              ...
              <div className="space-y-4">
                {[1, 2, 3, 4].map((item) => (
                  <div key={item} className="flex space-x-3 p-3 bg-gray-800 rounded-lg hover:bg-gray-750 cursor-pointer transition-colors">
                    <div className="w-16 h-16 bg-gray-600 rounded overflow-hidden flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <h4 className="text-white font-medium text-sm mb-1 truncate">{service.title}</h4>
                      <p className="text-[#FF9500] text-xs mb-1">{service.categories.map(cat => getLabel(cat, SERVICE_CATEGORIES)).join(' / ')}</p>
                      <p className="text-[#FF9500] font-bold text-sm mb-1">
                        {new Intl.NumberFormat('vi-VN').format(service.amount)} VND
                      </p>
                      <p className="text-gray-400 text-xs">{service.areas.map(area => getLabel(area, SERVICE_AREAS)).join(', ')}</p>
                    </div>
                  </div>
                ))}
              </div>
