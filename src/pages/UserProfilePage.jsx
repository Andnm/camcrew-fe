...
import { User, Calendar, Bell, CreditCard, FileText, LogOut, Edit2, Camera, Save, X, Phone, Mail, MapPin, Cake, MessageCircle, Clock, Star, CheckCircle, AlertCircle } from 'lucide-react';
  ...
  const messages = [
    {
      ...
      message: 'Cảm ơn bạn đã lựa chọn villa của chúng tôi. Chúc bạn có kỳ nghỉ vui vẻ!',
      time: '2 giờ trước',
      unread: true
    },
    {
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                <span className={`px-3 py-1 rounded-full text-sm ${
                  booking.status === 'Hoàn thành' ? 'bg-green-600 text-white' :
                  booking.status === 'Sắp tới' ? 'bg-blue-600 text-white' :
        ...
        {rentals.map((rental) => (
          <div key={rental.id} className="bg-gray-700 rounded-lg p-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="text-lg font-semibold text-white mb-1">{rental.title}</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                ...
                <p className="text-white font-semibold text-lg">{rental.bookings}</p>
              </div>
              <div className="text-center">
                <p className="text-gray-400 text-sm">Doanh thu</p>
                <p className="text-white font-semibold text-lg">{rental.revenue}</p>
              </div>
              <div className="text-center">
                <p className="text-gray-400 text-sm">Đánh giá</p>
                <div className="flex items-center justify-center space-x-1">
                  <Star className="w-4 h-4 text-yellow-400 fill-current" />
                  <span className="text-white font-semibold">{rental.rating}</span>
                </div>
              </div>
              <div className="text-center">
          ...
          <aside className="w-full lg:w-80">
            <div className="bg-gray-800 rounded-lg p-6">
              <div className="text-center mb-6">
                ...
                <h2 className="text-white text-xl font-bold">{formData.fullName}</h2>
                <p className="text-gray-400">Khách</p>
                <div className="mt-2">
