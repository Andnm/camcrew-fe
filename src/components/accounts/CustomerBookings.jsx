...
export default function CustomerBookings() {
  ...
  const fetchData = async (page = pagination.page) => {
    ...
    try {
      const res = await listBookings({
        ...
        status: BOOKING_STATUS_OPTIONS.COMPLETED,
      });
      setItems(res?.data || []);
      setPagination((p) => ({
        ...p,
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= totalPages) fetchData(newPage);
  };
  ...
  return (
    <div className="space-y-4">
      {loading ? (
        ...
        <div className="text-center text-gray-400 py-8">Chưa có đơn đã hoàn thành</div>
      ) : (
        items.map((b) => {
